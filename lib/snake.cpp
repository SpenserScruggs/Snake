...
void Frame::add_apple(std::vector<Pixel> &pixels){
    std::vector<Pixel*> not_player_or_apple;

    for(int i = 0; i < pixels.size(); i++){
        if(pixels[i].value == 4){
            not_player_or_apple.push_back(&pixels[i]);
        }
    }

    if(!not_player_or_apple.size()){
        return;
    }

    srand(time(0));

    int x = rand() % not_player_or_apple.size();

    not_player_or_apple[x]->value = 5;
}
...
void Frame::draw(){
    ...
    for(int i = 0; i < pixel_list.size(); i++){
        ...
        if(current_player.player_pos == pixel_list[i].pix_pos){
            if(pixel_list[i].value == 5){
                current_player.player_length++;
                add_apple(pixel_list);
            }
